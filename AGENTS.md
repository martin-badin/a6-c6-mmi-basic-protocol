...
- If you use an LLM agent, provide it with a single representative frame plus
  parsing code (e.g. `example/src/Command31.ts`) so it can infer correct bit/byte
  semantics.
...
1. Collect frames from `example/data.csv`, generate per-command markdown, and
   add them to `docs/commands/`.
...
If desired, I can scaffold a generator that emits files into `docs/commands/`
using the existing `example/src/` parsers as examples.
